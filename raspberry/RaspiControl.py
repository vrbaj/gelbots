...
class RaspiControl:
    ...
    def init_serial(self):
        ...
        self.ser.close()  # close the serial port which is open by default but with wrong parameters
        self.ser.open() 
    
    def move_xy(self, steps):
        GPIO.output(self.coilx_enable, GPIO.HIGH)
        GPIO.output(self.coily_enable, GPIO.HIGH)
        if self.ser.isOpen():
            try:
                text_command = "!0L" + steps +",0\r\n"
                command = bytes(text_command, "utf-8")
                # command = bytes(b"!0L-268,268,0\r\n")
                print("Sending data to motor..." + steps)
    ...
    def stamp(self,dx, dy, steps_x, steps_y, delay_x, delay_y, light_on, light_off, sfl_on, sfl_off, batch, stop_pill):
        for i in range(batch):
            ...
            time.sleep(0.05)
            time.sleep(1)
            print("moving back in x")
            self.move_xy(str(steps) + ",0")
            #for i in range(abs(steps)):
            #    GPIO.output(self.coilx_step, 1)
            #    time.sleep(0.001)
            #    GPIO.output(self.coilx_step, 0)
